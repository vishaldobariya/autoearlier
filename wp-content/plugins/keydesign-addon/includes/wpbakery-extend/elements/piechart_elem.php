...
if (!class_exists('KD_ELEM_PIE_CHART')) {
    ...
    class KD_ELEM_PIE_CHART extends KEYDESIGN_ADDON_CLASS {
        ...
        public function kd_piechart_shrt($atts, $content = null) {
            ...
            if( $pc_center_content == 'center_icon' && strlen($icon_iconsmind) > 0 ) {
            ...
            if ( '' == $pc_main_color ) {
              $pc_main_color = sway_get_option( 'tek-main-color' );
            }

            if ( '' == $pc_track_color ) {
              $pc_track_color = '#cfd5e5';
            }

            if( $pc_center_content == 'center_icon' && !empty($pc_icons) ) {
      				$content_icon = '<i class="'.$pc_icons .'" style="'.$icon_size_style.' '.$icon_color_style.'"></i>';
      			}
            ...
            $output .= '<div class="kd_pie_chart '.$css_animation.' '.$pc_extra_class.'" '.$animation_delay.'>';
              $output .='<span class="kd_chart" data-bar-color="'.$pc_main_color.'" '.(!empty($pc_track_color) ? 'data-track-color="'.$pc_track_color.'"' : '').' data-line-width="16" data-percent="'.$pc_value.'">';
