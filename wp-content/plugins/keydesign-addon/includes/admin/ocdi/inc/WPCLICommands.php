...
class WPCLICommands extends \WP_CLI_Command {
	...
	public function list_predefined() {
		...
		foreach ( $this->ocdi->import_files as $index => $import_file ) {
			WP_CLI::log( sprintf(
				...
				empty( $import_file['import_innerpages_file_url'] ) && empty( $import_file['local_import_innerpages_file'] ) ? 'no' : 'yes',
	...
	public function import( $args, $assoc_args ) {
		...
		if ( ! empty( $assoc_args['innerPages'] ) ) {
			$this->import_content( $assoc_args['innerPages'] );
		}
	...
	private function import_predefined( $predefined_index ) {
		...
		if ( ! empty( $import_files['innerPages'] ) ) {
			$this->import_content( $import_files['innerPages'] );
		}
