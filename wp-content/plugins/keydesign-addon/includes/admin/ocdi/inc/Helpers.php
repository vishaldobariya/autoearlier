...
class Helpers {
	 ...
	 */
	public static function download_import_files( $import_file_info, $importForm = false ) {
		...
		if ( $importForm && !empty( $importForm->homepage ) && $importForm->homepage ) {
			// Check if 'import_file_url' is not defined. That would mean a local file.
		...
		// Get Inner Pages import file
		if ( $importForm && !empty( $importForm->innerPages ) && $importForm->innerPages ) {
			if ( !empty( $import_file_info['local_import_innerpages_file'] ) ) {
				if ( file_exists( $import_file_info['local_import_innerpages_file'] ) ) {
					$downloaded_files['innerPages'] = $import_file_info['local_import_innerpages_file'];
				}
			}
		}

		// ----- Set widget file path -----
		if ( $importForm && !empty( $importForm->widgets ) && $importForm->widgets ) {
		...
		if ( $importForm && !empty( $importForm->themeOptions ) && $importForm->themeOptions ) {
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$innerPages_file_info = isset( $_FILES['innerPages_file'] ) ?
			wp_handle_upload( $_FILES['innerPages_file'], $upload_overrides ) :
			$file_not_provided_error;
		...
		// Process inner pages import file.
		if ( $innerPages_file_info && ! isset( $innerPages_file_info['error'] ) ) {
			// Set uploaded content file.
			$selected_import_files['innerPages'] = $innerPages_file_info['file'];
		}
		else {
			// Add this error to log file.
			$log_added = self::append_to_file(
				sprintf(
					__( 'Inner pages file was not uploaded. Error: %s', 'pt-ocdi' ),
					$innerPages_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'pt-ocdi' )
			);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		sprintf(
			...
			empty( $selected_import_files['innerPages'] ) ? esc_html__( 'not defined!', 'pt-ocdi' ) : $selected_import_files['innerPages'],
